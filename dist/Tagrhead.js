(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("Tagrhead", [], factory);
	else if(typeof exports === 'object')
		exports["Tagrhead"] = factory();
	else
		root["Tagrhead"] = factory();
})(this, function() {
...
var AutoSuggestControl = function () {
  function AutoSuggestControl(textbox, container, provider, tagsCtrl) {
  ...
  _createClass(AutoSuggestControl, [{
    key: 'getTags',
    ...
    key: 'autosuggest',
    value: function autosuggest(suggestions, typeAhead) {
      var _this = this;
      ...
      if (suggestions.length > 0) {
        ...
        suggestions.forEach(function (suggestion) {
          suggestion.results.forEach(function (item) {
            results.push(item);
          });
        });
        results = results.filter(function (suggestion) {
          return suggestion.itemValue.indexOf(_this.textbox.value) === 0;
        });
    ...
    key: 'selectRange',
    value: function selectRange(startPos, selectLength) {
    ...
    key: 'typeAhead',
    value: function typeAhead(suggestion) {
    ...
    key: 'createCriteriaLayer',
    value: function createCriteriaLayer() {
      var _this2 = this;
      ...
      this.criteriaLayer.onmousedown = this.criteriaLayer.onmouseup = this.criteriaLayer.onmouseover = function (event) {
    ...
    value: function showCriteriaLayer() {
      var _this3 = this;

      var suggestions = this.provider.requestHeaders();
      ...
      suggestions.forEach(function (datasetName) {
        div = null;
        div = document.createElement('div');
        ...
        _this3.criteriaLayer.appendChild(div);
      });
      if (this.style === 'mdl') {
    ...
    key: 'createDropDownLayer',
    value: function createDropDownLayer() {
      var _this4 = this;
      ...
      this.layer.onmousedown = this.layer.onmouseup = this.layer.onmouseover = function (event) {
        event = event || window.event;
        var target = event.target || event.srcElement;

        if (event.type === 'mousedown') {
          self.textbox.value = target.firstChild.nodeValue;
          _this4.tagsCtrl.addTags(_this4.tag);
    ...
    key: 'hideDropDownLayer',
    value: function hideDropDownLayer() {
      this.layer.style.visibility = 'hidden';
    }
    ...
    key: 'showDropDownLayer',
    value: function showDropDownLayer(suggestions) {
      var _this5 = this;

      this.layer.innerHTML = ''; // clear contents of the layer
      suggestions.forEach(function (dataset) {
        var div = null;

        div = document.createElement('div');
        div.classList.add('tagrhead-header');
        div.appendChild(document.createTextNode(dataset.name));
        _this5.layer.appendChild(div);
    ...
    value: function keyDownHandler(event) {
      if (this.mode === 'suggestions') {
        ...
        switch (event.keyCode) {
          ...
          case 13:
            // enter
            var criteriaNodes = this.criteriaLayer.childNodes;

            for (var i = 0; i < criteriaNodes.length; i++) {
              var node = criteriaNodes[i];
    ...
    key: 'highlightNextCriteria',
    value: function highlightNextCriteria() {
      var criteriaNodes = this.criteriaLayer.childNodes;

      if (criteriaNodes.length > 0 && this.criteriaCur < criteriaNodes.length - 1) {
        var node = criteriaNodes[++this.criteriaCur];
    ...
    key: 'highlightCriteria',
    value: function highlightCriteria(criteriaNode) {
      for (var i = 0; i < this.criteriaLayer.childNodes.length; i++) {
        var node = this.criteriaLayer.childNodes[i];
    ...
    key: 'highlightPreviousCriteria',
    value: function highlightPreviousCriteria() {
      var criteriaNodes = this.criteriaLayer.childNodes;
    ...
    key: 'highlightPreviousSuggestion',
    value: function highlightPreviousSuggestion() {
      var suggestionNodes = this.layer.childNodes;
    ...
    value: function highlightSuggestion(suggestionNode) {
    ...
    value: function init() {
      ...
      this.textbox.onkeyup = function (event) {
        ...
        if (_this6.textbox.value === '') {
          _this6.mode = 'criteria';
          _this6.hideDropDownLayer();
        } else {
...
exports.default = AutoSuggestControl;
...
var _bloodhoundMin = __webpack_require__(4);

var _bloodhoundMin2 = _interopRequireDefault(_bloodhoundMin);
...
var SuggestionProvider = function () {
  function SuggestionProvider(dataSources) {
  ...
  _createClass(SuggestionProvider, [{
    ...
    value: function _createBloodHoundConfig(dataSource) {
      return {
        local: dataSource.data,
        datumTokenizer: function datumTokenizer(data) {
          return _bloodhoundMin2.default.tokenizers.whitespace(data[dataSource.srchKey]);
        },

        queryTokenizer: _bloodhoundMin2.default.tokenizers.whitespace,
        identify: function identify(obj) {
          return obj[dataSource.srchKey];
        },
        limit: 50
      };
    }
  }, {
    key: '_createBloodHound',
    value: function _createBloodHound(dataSource) {
      var config = this._createBloodHoundConfig(dataSource);
      var engine = new _bloodhoundMin2.default(config);

      var srchResults = void 0;

      var sync = function sync(datums) {
        // console.log('datums from `local`, `prefetch`, and `#add`')
        // console.log(datums)
        srchResults = datums;
      };

      var async = function async(datums) {
    ...
    key: 'init',
    value: function init(dataSources) {
      var _this = this;

      this.bloodHoundsData = dataSources.map(function (dataSource) {
        return _this._createBloodHound(dataSource);
      });

      var bloodhounds = this.bloodHoundsData.map(function (item) {
        return item.bloodHound.initialize();
      });

      Promise.all(bloodhounds).then(function () {/* console.log('bloodhounds ready to go') */}).catch(function (err) {
    ...
    value: function requestHeaders() {
      return this.bloodHoundsData.map(function (dataSource) {
        return dataSource.itemHeader;
      });
    }
  }, {
...
var Tags = function () {
  function Tags(containerId, inputId, config) {
  ...
  _createClass(Tags, [{
    key: 'removeBtnHandler',
    value: function removeBtnHandler(event) {
      ...
      if (event.target.classList.contains('tagrhead-tag__remove')) {
        ...
        var tagValueNode = _$('.tagrhead-tag__value', tag);

        this.tagsContainer.removeChild(tag);
        var index = -1;

        this.tagsArr.forEach(function (item, idx, arr) {
    ...
    value: function keyHandler(event) {
      if (event.target.tagName === 'INPUT') {
        ...
        if (keyCode !== this.KEYS.BACK) {
          var tags = _$$('.tagrhead-tag__marked', this.tagsContainer);
        ...
        var newTagValue = target.value.trim();
    ...
    key: 'createBasicTag',
    value: function createBasicTag(value, key, header) {
      key = key || value;
      header = header || value;

      var tag = _create('div', {
        'className': 'tagrhead-tag',
        'innerHTML': '<span class="tagrhead-tag__value" data-key="' + key + '" data-value="' + value + '" data-header="' + header + '" >' + '<span>' + header + '-' + value + '</span>' + '<button class="tagrhead-tag__remove">&times;</button>' + '</span>'
      });
    ...
    key: 'createMdlTag',
    value: function createMdlTag(value, key, header) {
      key = key || value;
      header = header || value;

      var tag = _create('div', {
        'className': 'tagrhead-tag',
        'innerHTML': '<span class="tagrhead-tag__value mdl-chip mdl-chip--deletable"' + 'data-key="' + key + '" data-value="' + value + '" data-header="' + header + '" >' + '<span class="mdl-chip__text">' + header + '-' + value + '</span>' + '<button class="tagrhead-tag__remove mdl-chip__action">' + '<i class="tagrhead-tag__remove material-icons">cancel</i>' + '</button>' + '</span>'
      });

      return tag;
    }
  }, {
    key: 'createBootstrapMdTag',
    value: function createBootstrapMdTag(value, key, header) {
      ...
      var tag = _create('div', {
        'className': 'tagrhead-tag',
        'innerHTML': '<a class="tagrhead-tag__value btn btn-info" href="javascript:void(0)"' + 'data-key="' + key + '" data-value="' + value + '" data-header="' + header + '" >' + header + '-' + value + '<span class="tagrhead-tag__remove badge">' + 'x' + '</span>' + '</a>'
      });
    ...
    value: function removeTag() {
      ...
      var tags = _$$('.tagrhead-tag', this.tagsContainer);
    ...
    key: 'addTag',
    value: function addTag(value, key, header) {
      var _this = this;
      ...
      if (~this.tagsArr.map(function (item) {
        return item.value;
      }).indexOf(value)) {
        var exist = _$$('tagrhead-tag__exists', this.tagsContainer);

        Array.prototype.forEach.call(exist, function (tag) {
          if (_this.transitionEnd) {
            _oneListener(tag, _this.transitionEnd, function () {
      ...
      var tag = this.createTag(value, key, header);
    ...
    value: function addTags(data) {
      ...
      if (Array.isArray(data)) {
        data.forEach(function (item) {
          _this2.addTag(item.value, item.key, item.header);
        });
      } else {
...
var _AutoSuggestControl = __webpack_require__(0);

var _AutoSuggestControl2 = _interopRequireDefault(_AutoSuggestControl);

var _SuggestionProvider = __webpack_require__(1);
...
var Tagrhead = function () {
  function Tagrhead(inputId, containerId, dataSources, config) {
    _classCallCheck(this, Tagrhead);

    this.textbox = new _AutoSuggestControl2.default(document.getElementById(inputId), document.getElementById(containerId), new _SuggestionProvider2.default(dataSources), new _Tags2.default('#' + containerId, '#' + inputId, config), config);
  }

  _createClass(Tagrhead, [{
    key: 'getTags',
    value: function getTags() {
...
!function(a,b){ true?!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(9)], __WEBPACK_AMD_DEFINE_RESULT__ = function(c){return a.Bloodhound=b(c)}.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__),
...
//# sourceMappingURL=Tagrhead.js.map